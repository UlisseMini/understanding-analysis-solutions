...
\begin{solution}
  By the density theorem we first find $a < p/q < b$, then we add $(\sqrt2)/n$ with $n$ small enough that $a < (p/q) + (\sqrt2)/n < b$. By (a) we know $(p/q) + (\sqrt2)/n \in \mathbf{I}$. Thus we have found $t = (p/q) + (\sqrt2)/n$ with $a < t < b$ and $t \in \mathbf{I}$ which proves Corollary 1.4.4.

  (I'm not sure what the exercise wanted me to do with $a-\sqrt2$ and $b-\sqrt2$. I'll figure out later) \newline
