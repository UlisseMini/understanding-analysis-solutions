...
\begin{solution}
  ...
  \item True. First suppose $a < b + \epsilon$ forall $\epsilon > 0$, We want to show this implies $a \le b$. We either have $a \le b$ or $a > b$, but $a > b$ is impossible since the gap implies there exists an $\epsilon$ small enough such that $a > b + \epsilon$.

    Second suppose $a \le b$, obviously $a < b + \epsilon$ forall $\epsilon > 0$.
  }
\end{solution}
...
\begin{solution}
  \enum{
  \item There exist real numbers satisfying $a < b$ where $a + 1/n \ge b$ forall $n \in \mathbf N$ (false).
  \item For every real number $x > 0$ there exists an $n \in \mathbf N$ such that $x < 1/n$ (true).
  \item There exist two real numbers $a < b$ such that if $r < b$ then $r < a$ forall $r \in \mathbf Q$ (false).
  }
\end{solution}
