...
\begin{solution}
  ...
  \item $A = \{r \in \mathbf{Q} \mid r^3 < 2\}$, $B = A^c$ does not satisfy the cut property in $\mathbf{Q}$ since $\sqrt[3]{2} \notin \mathbf{Q}$.

    Note: Using $A = \{r \in \mathbf{Q} \mid r^2 < 2\}$ isn't quite correct since values less than $-\sqrt 2$ wouldn't be in $A$.
  }
\end{solution}
