...
\begin{solution}
  ...
  \item Theorem: Suppose $i$ is a lower bound for $A$, it is the greatest lower bound if and only if forall $\epsilon>0$, there exists an $a\in A$ such that $i + \epsilon < a$. \par
    ...
    $(\impliedby)$ Suppose $j > i$ is a lower bound on $A$, then set $\epsilon = j-i$ and we have that $i+\epsilon = j$ is not a lower bound since there exists $a \in A$ such that $j > a$. Therefor $i$ is the greatest lower bound.
  }
\end{solution}
...
\begin{solution}
  ...
  \item Yes. Let $S = \left\{\sup A_k \mid k=1,\dots,\right\}$ and $s = \sup S$. $s$ is obviously an upper bound for $\bigcup_{k=1}^\infty A_k$. to see it is the least upper bound suppose $s' < s$, then by definition there exists a $k$ such that $\sup A_k > s'$ implying $s'$ is not an upper bound for $A_k$. Therefor $s$ is the least upper bound.
  }
\end{solution}
...
% TODO: Solve (c), I found this quizlet solution
% https://quizlet.com/explanations/textbook-solutions/understanding-analysis-2nd-edition-9781493927128/chapter-1-exercises-6-47d0607f-d570-454e-b7a2-6113c474fe58
% But I think their solution is flawed, since it assumes sup(A + B) = u to derive s+t<=u
...
\begin{solution}
  ...
  \item From (a) we know $s + t$ is an upper bound, so we must only show it is the least upper bound. \TODO
  \item Showing $s + t - \epsilon$ is not an upper bound for any $\epsilon > 0$ proves it is the least upper bound by Lemma 1.3.8. Rearranging gives $(s - \epsilon/2) + (t - \epsilon/2)$ we know there exists $a > (s - \epsilon/2)$ and $b > (t - \epsilon/2)$ therefor $a + b > s + t - \epsilon$ meaning $s+t$ cannot be made smaller, and thus is the least upper bound.
  }
\end{solution}
