...
\begin{solution}
  \enum{
  \item True, $s = \sup A > \sup B$ implies there exists an $a$ such that $(\sup A) - \epsilon < a < \sup B$ which is impossible since $a \in A$ implies $a \in B$, but $a < \sup B$ implies $a \notin B$. Therefor $\sup A \le \sup B$.
  \item \TODO
  \item False, consider $A = \{x \mid x < 1\}$, $B = \{x \mid x > 1\}$. $c = 1$ works, but $\sup A \not < \inf B$.
  }
\end{solution}
