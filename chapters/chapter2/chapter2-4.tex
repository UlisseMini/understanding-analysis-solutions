...
\begin{solution}
  ...
  \item $x_2 = 1$ makes me conjecture $x_n$ is monotonic. For induction suppose $x_n > x_{n+1}$ then we have
    ...
    Therefor by the monotone convergence theorem $(x_n)$ converges.
...
\begin{solution}
  \enum{
  \item The series $y_n = (1, 2, 1, 2, \dots)$ does not converge.
...
\begin{solution}
  ...
  \item Let $x_1 = \sqrt 2$ and $x_{n+1} = \sqrt{2 + x_n}$ clearly $x_2 > x_1$. assuming $x_{n+1} > x_n$ gives
    ...
    $$
    x = \sqrt{2 + x} \iff x^2 - x - 2 = 0 \iff x = \frac 12 \pm \frac{\sqrt{5}}{2}
    $$
    Since $x > 0$ we must have $x = (1 + \sqrt 5)/2$.
  \item We have $x_1 = 2^{1/2}$, $x_{n+1} = (2x_n)^{1/2}$ taking logs we get $y_1 = 1/2$, $y_{n+1} = (1 + y_n)/2$. obviously $(y_n) \to 1$ therefor $(x_n) \to 2^1$.
...
\begin{solution}
  \enum{
  \item MCT tells us $(1/n)$ converges, obviously it must converge to zero therefor we have $|1/n - 0| = 1/n < \epsilon$ for any $\epsilon$, which is the Archimedean Property.
  \item We have $I_n = [a_n, b_n]$ with $a_n \le b_n$ since $I_n\ne \emptyset$. Since $I_{n+1} \subseteq I_n$ we must have $b_{n+1} \le b_n$ and $a_{n+1} \ge a_n$ the MCT tells us that $(a_n) \to a$ and $(b_n) \to b$. by the Order Limit Theorem we have $a \le b$ since $a_n \le b_n$, therefor $a \in I_n$ for all $n$ meaning $a \in \bigcap_{n=1}^\infty I_n$ and thus $\bigcap_{n=1}^\infty I_n \ne \emptyset$.
  }
\end{solution}
...
\begin{solution}
  ...
  \item Clearly $x_1^2 \ge 2$, now procede by induction. if $x_{n}^2 \ge 2$ then we have
    ...
    $$
    To show $x_n - x_{n+1} \ge 0$ we examine $(x_n - x_{n+1})^2 \ge 0$ with the hope of using $x_n^2 \ge 2$.
    $$
    \begin{aligned}
      (x_n - x_{n+1})^2
      &= x_n^2 - 2x_nx_{n+1} + x_{n+1}^2 \\
      &= x_n^2 - 2x_n \frac 12\left(x_n + \frac{1}{x_n}\right) + x_{n+1}^2 \\
      &= x_n^2 - (x_n^2 + 2) + x_{n+1}^2 \\
      &= x_{n+1}^2 - 2 \ge 0.
    \end{aligned}
    $$
    Now we know $(x_n) \to x$ converges, to show $x^2 = 2$ observe that $\lim x_n = \lim x_{n+1}$ so
    $$
    x = \frac 12 \left(x + \frac{2}{x}\right) \iff x^2 = \frac 12 x^2 + 1 \iff x^2 = 2
    $$
    Therefor $x = \pm \sqrt 2$, since every $x_n$ is positive $x = \sqrt 2$.
  \item Let
    $$
...
\begin{solution}
  ...
  \item We have
    ...
    $$
  \item The only fixed points are $x_n = y_n$ so we only need to show both sequences converge.
    \TODO
  }
\end{solution}
...
\begin{solution}
  ...
  \item This is a geometric series, we can use the usual trick to derive $s_n$. Let $r = 1/2$ for convenience
    ...
    = \lim_{n\to \infty} \frac{(1/2)^{n+1} - 1}{1/2 - 1}
    = \frac{-1}{-1/2} = 2
    $$
  ...
  \item Another telescoping series, since
    ...
    $$
    Therefor most of the terms cancel and we get
    $$
    s_n = \log(n+1)
    $$
...
\begin{solution}
  % Since $b_n \ge 0$ and $b_n$ are decreasing, we have
  % $$
  % b_1 + b_2 + (b_3 + b_4) > b_1 + b_2 + (b_4 + b_4)
  % $$
  % Since $b_4 < b_3$. In general

  % Therefor $\sum_{n=1}^\infty b_n$ diverges as it is smaller then $\sum_{n=0}^\infty 2^nb_{2^n}$ which diverges.

  \TODO
  The harmonic series is an example of this argument
  $$
  1 + \frac{1}{2} + \left(\frac 13 + \frac 14\right) > 1 + \frac 12 + \left(\frac 14 + \frac 14\right) = 2
  $$
  Since the right hand side becomes arbitrarily large and the harmonic series is greater then it, it must diverge.
\end{solution}
