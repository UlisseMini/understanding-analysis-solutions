...
\begin{solution}
  ...
  \item
    ...
    The reason you would think to set $m=n$ is in trying to maximize $mn/(m^2 + n^2)$ notice if $m>n$ then $mn>n^2$ so we are adding more to the numerator then the denominator, hence the ratio is increasing. And if $m<n$ then the ratio is decreasing. Therefor the maximum point is at $m=n$ as can be seen in \href{https://www.desmos.com/calculator/t993sievxl}{this} graph.
  \item \TODO
  \item
    We are given $|b_m - a_{mn}| < \epsilon$ for all $n > N$, By the triangle inequality
    $$
