...
\begin{solution}
  \enum{
  \item Divide by $a_n^2$, then apply the ALT
    $$
    \lim \left(\frac{1+2 a_{n}}{1+3 a_{n}-4 a_{n}^{2}}\right)
    = \frac{\lim\left(\frac{1}{a_n^2} + \frac{2}{a_n}\right)}{\lim\left(\frac{1}{a_n^2} + \frac{3}{a_n} - 4\right)}
    = \frac{0}{-4}
    = 0
    $$
  \item
...
\begin{solution}
  ...
  \item Impossible, the algebraic limit theorem implies $\lim (x_n + y_n) - \lim (x_n) = \lim y_n$ therefor $(y_n)$ must converge if $(x_n)$ and $(x_n + y_n)$ converge.
  \item Impossible, the algebraic limit theorem implies that if $(b_n) \to b$ then $(1/b_n) \to 1/b$.
...
\begin{solution}
  \enum{
  \item
    Let $D = \sup\{|x_n - x| : n \in \mathbf N\}$ we have
    $$
    |y_n - x| = \left|\frac{x_1 + \dots + x_n}{n} - x\right| \le \left|\frac{|x_1 - x| + \dots + |x_n - x|}{n}\right| \le D
    $$

    Let $|x_n - x| < \epsilon/2$ for $n \ge N_1$. We have
    $$
    |y_n - x| \le \left|\frac{|x_1-x| + \dots + |x_{N_1}-x| + \dots + |x_n-x|}{n}\right| \le \left|\frac{N_1 D + (n-N_1)\epsilon/2}{n}\right|
    $$
    Let $N_2$ be large enough that for all $n > N_2$
    $$
    \left|\frac{N_1 D + (n-N_1)\epsilon/2}{n}\right| = \left|\frac{N_1(D - \epsilon/2)}{n} + \epsilon/2\right| < \epsilon
    $$
    then let $N = \max\{N_1, N_2\}$.

    \TODO Make more rigorous, remove $\epsilon/2$ hack
...
\begin{solution}
  ...
  \item True, since if $a \in (0,1)$ then there would exist an $\epsilon$-neighborhood inside $(0,1)$ that $a_n$ would have to fall in (because its an open interval).
...
\begin{solution}
  ...
  \item
    ...
    For $a_{mn} = mn/(m^2 + n^2)$ limits are zero, and $\lim_{m,n\to \infty} a_{mn}$ does not exist since if $m,n \ge N$ setting $m=n=N$ gives
    $$
    \frac{N^2}{N^2 + N^2} = \frac{1}{2}
    $$
    Which cannot be made smaller then any $\epsilon$.

    The reason you would think to set $m=n$ is in trying to maximize $mn/(m^2 + n^2)$ notice if $m>n$ then $mn>n^2$ so we are adding more to the numerator then the denominator, hence the ratio is increasing. And if $m<n$ then the ratio is decreasing. Therefor the maximum point is at $m=n$ as can be seen in \href{https://www.desmos.com/calculator/t993sievxl}{this} desmos graph.
  \item Geometrically we want the point $(m,n)$ to be arbitrarily small for $m,n \ge N$ but not arbitrarily small for $m\to \infty$ with $n$ fixed. \TODO
  \item \TODO
  \item \TODO
  }
\end{solution}
