...
\begin{solution}
  ...
  \item
    ...
    The reason you would think to set $m=n$ is in trying to maximize $mn/(m^2 + n^2)$ notice if $m>n$ then $mn>n^2$ so we are adding more to the numerator then the denominator, hence the ratio is increasing. And if $m<n$ then the ratio is decreasing. Therefor the maximum point is at $m=n$.
  \item \TODO
  \item
    We need to find $N$ so that $|b_m - a| < \epsilon$ for all $m > N$.
    Set $N$ such that $|a_{mn} - a| < \epsilon/2$ when $n,m \ge N$. Fix $m \ge N$ and apply the triangle inequality
    $$
    |b_m - a| \le |b_m - a_{mn}| + |a_{mn} - a|\quad\forall n \in \mathbf{N}
    $$
    Set $n \ge N_m$ where $N_m$ (dependent on $m$) is large enough that $|b_m - a_{mn}| < \epsilon/2$. Then
    $$
    |b_m - a| \le |b_m - a_{mn}| + |a_{mn} - a| < \epsilon/2 + \epsilon/2 = \epsilon
    $$

    We're allowed to pick $n$ depending on $m$ because the triangle inequality holds \emph{forall n}.
    In the end all we did was find $N$ and show that every $m \ge N$ has $|b_m - a| < \epsilon$.
