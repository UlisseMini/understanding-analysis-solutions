...
\[\abs{e^{-\alpha t} - e^{-\alpha_0 t_0}} \leq \abs{e^{\alpha_0 t_0} - e^{\alpha_0 t}} + \abs{e^{\alpha_0 t} - e^{\alpha t}} < \epsilon\]
Since \(e^{xy}\) is continuous with respect to \(x\) for a fixed \(y\), we  can find \(\delta_t\) and \(\delta_\alpha\) so that if \(\abs{t_0 - t} < \delta_t\) and \(\abs{\alpha_0 - \alpha} < \delta_\alpha\) then
\[\abs{e^{\alpha_0 t_0} - e^{\alpha_0 t}} < \frac{\epsilon}{2} \text{ and }   \abs{e^{\alpha_0 t} - e^{\alpha t}} < \frac{\epsilon}{2}\]
We can ensure both of these conditions by setting \(\delta = \min \{\delta_t, \delta_\alpha\}\), thus showing \(f(\alpha, t)\) is continuous.
