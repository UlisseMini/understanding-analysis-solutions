...
\begin{solution}
  \enum{
  \item Since $f$ is continuous $f(0) = \lim f(x_n) = 0$. For $f'$ note $f(x_n) = 0$ implies
    $$f'(0) = \lim_{x \to 0} \frac{f(x)}{x} = \lim \frac{f(x_n)}{x_n} = 0$$
    A subtle bit is how $f'(0)$ existing implies the limit exists, normally a single sequence converging would not be enough.
...
\begin{solution}
  ...
  $$
  But we know $f'(c) \ne 1$, therefor finding more then one fixed point is impossible.
\end{solution}
