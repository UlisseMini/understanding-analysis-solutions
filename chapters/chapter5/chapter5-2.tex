...
\begin{solution}
  ...
  \item We have
    ...
    \item We need to show that \(\lim_{x \to y} f'(x) = f'(y)\). Now, if we choose \(\delta\) small enough such that when \(\abs{x - y} < \delta\), both
    \[\abs{f'(x) - \frac{f(x) - f(y)}{x - y}} < \frac{\epsilon}{2}\]
    and
    \[\abs{\frac{f(x) - f(y)}{x - y} - f'(y)} < \frac{\epsilon}{2}\]
    (this is possible because \(f\) is uniformly differentiable), then we have
    \[\begin{aligned}
    f'(x) - f'(y) &= f'(x) -\frac{f(x) - f(y)}{x - y} + \frac{f(x) - f(y)}{x - y} - f'(y) \\
    & \leq \abs{f'(x) - \frac{f(x) - f(y)}{x - y}} +\abs{\frac{f(x) - f(y)}{x - y} - f'(y)}  < \epsilon
    \end{aligned}
    \]
    as desired.
