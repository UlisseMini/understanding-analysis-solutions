...
\begin{solution}
\enum{
    \item Let the polynomial \(p(x)\) be the partial sum of the Taylor series of \(\sqrt{1-x}\) which satisfies \(\abs{p(x) - \sqrt{1-x}} < \epsilon\), and let \(q(x) = p(1-x^2)\). We then have \(\abs{q(x) - \sqrt{1-(1-x^2)}} = \abs{|x| - q(x)} < \epsilon\) as desired.
