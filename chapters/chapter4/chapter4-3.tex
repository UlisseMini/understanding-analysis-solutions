...
\begin{solution}
  Let $(x_n)$ be a convergent sequence in $K$ and set $x = \lim x_n$. Since $h$ is continuous the limit $\lim h(x_n) = h(x)$ and since $h(x_n) = 0$ forall $n$ $\lim h(x_n) = 0$. therefore $h(x) = 0$ and $x \in K$.
\end{solution}
