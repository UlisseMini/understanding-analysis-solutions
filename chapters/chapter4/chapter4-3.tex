...
\begin{solution}
  ...
  \item Let $g(x) = 1/f(x)$, we are given that $f(x)$ is not continuous at zero, and
    $$
    \lim_{x \to 0} \left[f(x) + g(x)\right] = f(0) + g(0)
    $$
    Therefor $g(0)$ exists, and so know that $g(x)$ is not continuous at zero since if it was $1/g(x) = f(x)$ would be continuous at zero.
...
\begin{solution}
  Let $(x_n)$ be a convergent sequence in $K$ and set $x = \lim x_n$. Since $h$ is continuous the limit $\lim h(x_n) = h(x)$ and since $h(x_n) = 0$ forall $n$ $\lim h(x_n) = 0$. Therefor $h(x) = 0$ and $x \in K$.
\end{solution}
...
\begin{solution}
  ...
  \item We can reason by cases. if $x = 0$ then $f_n(0) = 0$ for all $n$ so $h(0) = 0$. If $x \ne 0$ then $|x| > 1/n$ for all $n > N$ meaning we have $h(x) = \max\{f_1(x), \dots, f_N(x), 1\}$. Since $n|x| < 1$ for all $|x| < 1/n$ we have $h(x) = 1$ and so
    ...
    \end{cases}
    $$
    Which is not continuous at $x = 0$, therefor (a) does not hold in the infinite case.
  }
\end{solution}
