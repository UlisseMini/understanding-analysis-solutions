...
\begin{solution}
  \enum{
    \item Using the notation from Exercise 3.2.14, note that \(F^o,F^c,\) and \(\overline{F^c} / F^c\) are disjoint but their union is \(\mathbf{R}\); moreover \(F^o \cup \overline{F^c} / F^c = F\). Let \(d(x)\) denote Dirichlet's function (1 on rationals, 0 on irrationals), and consider
    \[ f(x) = \begin{cases}
        d(x) & x \in F^o \\
