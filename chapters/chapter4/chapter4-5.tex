...
\begin{solution}
  ...
  Define $A = \{x \in (a,b) : f(x) \le L\}$ and $B = \{x \in (a,b) : f(x) > L\}$. $A,B$ are clearly disjoint, they are also nonempty since $a+\epsilon \in A$ and $b-\epsilon \in B$ for small $\epsilon$. Since $(a,b)$ is connected there exists a sequence in $A$ or $B$ with limit in the other. Let $(x_n)$ be such a sequence, I claim $f(x_n) \to L$ and thus $(x_n) \to x$ with $f(x) = L$.
  ...
  Note that $(x_n)$ in $A$ cannot have a limit in $B$ since $f(x) \le L$ disallows $f(x) > L$. In other words $A$ is closed.
\end{solution}
