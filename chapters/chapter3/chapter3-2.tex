...
\begin{solution}
  ...
  \item Open. \(A^c\) is closed, so \(\closure{A^c} = A^c\). Also, \(\closure{A} \supseteq A\), so \(\closure{A}^c \subseteq A^c\). Hence, \(\closure{A}^{c} \cap \closure{A^{c}} = \closure{A}^{c}\) is open.
  }
\end{solution}
