...
\begin{solution}
  ...
  Now take $P = \mathbf{R}$, we get $P \cap K = K$ which is not necessarily perfect.
\end{solution}
...
\begin{solution}
  ...
  \[[L, L + 2\zeta_1) \backslash \bigcup^{n+1}_{i=1} V_{\zeta_i} (x_i) \subseteq [L, L + 2\zeta_{n+1}) \backslash V_{\zeta_{n+1}}(x_{n+1}) = [L, L + 2\zeta_{n+1}) \backslash (\alpha_{n+1}, \beta_{n+1})\]
...
Also, since \(\alpha_n > L \  \forall n\), \(L \in [L, L + 2\zeta_1) \backslash \bigcup^\infty_{i=1}V_{\zeta_i} (x_i)\). This completes the proof of the lemma.
  ...
  \TODO - the below construction is incorrect; rational numbers at the endpoints of each neighborhood will never be included. See discussion in issue \# 84
